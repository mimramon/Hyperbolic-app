...
class PlotCanvas extends DoubleBufferedCanvas implements MouseListener, MouseMotionListener
{
    ...
    public void paint(Graphics gfx) {
        ...
        if(ACTIVE==1) {
            ...
            if(S1.S[9].select==1) {C_set(1,S1.S[9].C);
                currentRender(g);}
            if(S2.S[9].select==1) {C_set(2,S2.S[9].C);
                currentRender(g);}
            if(S1.S[10].select==1) {E_set(2,S1.S[10].C);
                currentRender(g);}
            if(S1.S[11].select==1) {Q_set(2,S1.S[11].C);
                currentRender(g);}
            if(S2.S[10].select==1) {E_set(1,S2.S[10].C);
                currentRender(g);}
            if(S2.S[11].select==1) {Q_set(1,S2.S[11].C);
                currentRender(g);}
            if(S1.S[14].select==1) {HQ_set(S1.S[14].C);
                currentRender(g);}
            if(S2.S[14].select==1) {HQ_set(S2.S[14].C);
                currentRender(g);}
            if(S1.S[15].select==1) {R_set(1,S1.S[15].C);
                currentRender(g);}
            if(S2.S[15].select==1) {R_set(2,S2.S[15].C);
                currentRender(g);}
            if(S1.S[18].select==1) {QS_set(2,S1.S[18].C);
                currentRender(g);}
            if(S2.S[18].select==1) {QS_set(1,S2.S[18].C);
                currentRender(g);}

            if(S1.S[7].select==1) {E_set(0,S1.S[7].C);
                currentRender(g);}
            if(S2.S[7].select==1) {E_set(0,S2.S[7].C);
                currentRender(g);}
            if(S1.S[8].select==1) {Q_set(0,S1.S[8].C);
                currentRender(g);}
            if(S2.S[8].select==1) {Q_set(0,S2.S[8].C);
                currentRender(g);}
    ...
    void S_set(int q,int k,Color C,double POS) {
        ...
        if((k<=3)&&(TYPE==1)) {lim1=1;lim2=n-1;num=n-1;}
